...
try {
    ...
    throw new Error('Failed to read RSA key files. Please ensure keys/private.pem and keys/public.pem exist and are accessible.');
}
const systemAESKey: string = process.env.SYSTEM_AES_KEY;

if (!systemAESKey) throw new Error('SYSTEM_AES_KEY not found in environment variables');
if (!privateKey) throw new Error('Private key not found in environment variables, please make sure keys/private.pem exists and is accessible');
if (!publicKey) throw new Error('Public key not found in environment variables, please make sure keys/public.pem exists and is accessible');
...
async function getUserRecord(studentID: string): Promise<UserCryptoKeyModel> {
    ...
    if (!record) {
        throw new ErrorHandler(404, 'User AES key not found, Have you registered it yet?');
    }
    return {
...
function decryptWithAES(encryptedData: string, aesKey: string): string {
    try {
        ...
        throw new ErrorHandler(400, 'AES Decryption failed: ' + (error as Error).message);
    }
}
...
export function decryptWithRSA(encryptedData: string): string {
    try {
        ...
        throw new ErrorHandler(400, 'RSA Decryption failed: ' + (error as Error).message);
    }
}

export async function createUserCryptoInfo(payload: RegisterUserCryptoPayload): Promise<void> {
...
export async function registerUserCryptoInfo(encryptedPayload: string): Promise<void> {
    // 先 RSA 解密
    const decryptedStr = decryptWithRSA(encryptedPayload);

    let payload: RegisterUserCryptoPayload;
    try {
        payload = JSON.parse(decryptedStr);
    } catch (error) {
        throw new ErrorHandler(400, 'Invalid register payload JSON: ' + (error as Error).message);
    }

    // 驗證是否已註冊
    const existing = await UserCryptoKey.findOne({ where: { studentID: payload.studentID } });
    if (existing) {
...
export async function decryptToken(encryptedToken: string, studentID: string, userAESKey: string): Promise<UserAccessTokenPayload> {
    ...
    try {
        ...
        throw new ErrorHandler(400, 'Failed to parse decrypted token JSON: ' + (error as Error).message);
    }
}
...
export async function decryptNVerrifyUserAccessToken(
    ...
    const decryptedToken = await decryptToken(encryptedToken, studentID, userRecord.aesKey);
    const isVerified = verifyUserAccessToken(studentID, decryptedToken, userRecord);
    if (!isVerified) {
